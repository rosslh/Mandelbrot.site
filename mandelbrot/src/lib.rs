...
/// Checks if the entire rectangle is within the Mandelbrot set
fn is_region_fully_in_mandelbrot_set(
    re_range: itertools_num::Linspace<f64>,
    im_range: itertools_num::Linspace<f64>,
    max_iterations: u32,
    exponent: u32,
) -> bool {
    rect_in_set(re_range.clone(), im_range.clone(), max_iterations, exponent)
}
...
pub fn get_mandelbrot_set_image(
    ...
    if is_region_fully_in_mandelbrot_set(
        re_range.clone(),
        im_range.clone(),
        max_iterations,
        exponent,
